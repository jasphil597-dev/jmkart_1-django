...
def ensure_user_profile(user):
    """
    Make sure a UserProfile exists for this user.
    Prevents: UserProfile.DoesNotExist errors on older accounts.
    """
...
def register(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # Create User Profile
            profile = UserProfile()
            profile.user_id = user.id  # type: ignore
...
def login(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            # ✅ Ensure profile exists (in case older account has no profile)
...
def activate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        # ✅ Ensure profile exists after activation too
        ensure_user_profile(user)
...
def dashboard(request):
    ...
    # ✅ FIX: don't crash if profile is missing
    userprofile = ensure_user_profile(request.user)
...
def forgotPassword(request):
    if request.method == "POST":
        ...
        if Account.objects.filter(email=email).exists():  # type: ignore
...
def edit_profile(request):
    # ✅ FIX: don't 404 if profile is missing
    userprofile = ensure_user_profile(request.user)
